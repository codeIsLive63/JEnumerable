...
*   **Совместимость с Stream API:** Позволяет использовать цепочки операций, включая возможность работы с `Stream`.
...
3.  Соберите проект, используя предпочитаемый инструмент сборки (например, Maven или Gradle). Поскольку предоставленный код не включает сценарии сборки, вам нужно будет создать их самостоятельно на основе настроек проекта. Например, с Maven:

    ```bash
    mvn clean install
    ```

    (Нужно создать файл pom.xml в корне проекта. Базовый файл pom.xml не включен.)
